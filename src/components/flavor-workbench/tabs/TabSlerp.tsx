...
export function TabSlerp() {
  ...
  return (
    <TabSlerpInner
      ingA={ingA} ingB={ingB}
      vectorA={vectorA} vectorB={vectorB}
      setVectorA={setVectorA} setVectorB={setVectorB}
...
function TabSlerpInner({ ingA, ingB, vectorA, vectorB, setVectorA, setVectorB, activeIngredients }: TabSlerpInnerProps) {
  ...
  const visibleIngredients = useMemo(() => {
    ...
    const maxVisible = Math.floor(15 + (zoom - 1) * 25);
  ...
  const autoFitView = useCallback(() => {
    ...
    const newZoom = Math.min(600 / contentWidth, 380 / contentHeight);
    setZoom(Math.max(0.6, Math.min(newZoom, MAX_ZOOM)));
  }, [autoFitEnabled, ingA, ingB, neighbors]);

  useEffect(() => { autoFitView(); }, [vectorA, vectorB, autoFitView]);
  ...
  const corridorPanorama = useMemo(() => {
    const allIngredients = getIngredients().map(i => ({
      id: i.id, name: i.name, nameEn: i.nameEn,
      pos: { x: i.pca[0], y: i.pca[1] } as Vec2, category: i.category,
    }));
    return TIMELINE_STOPS.map(stop => {
      const pos = slerp2d({ x: ingA.pca[0], y: ingA.pca[1] }, { x: ingB.pca[0], y: ingB.pca[1] }, stop);
  ...
  // Adaptive PCA→SVG mapping
  const pcaRange = Math.max(Math.abs(ingA.pca[0] - ingB.pca[0]), Math.abs(ingA.pca[1] - ingB.pca[1]), 1);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Upper section: scatter + controls */}
          ...
          <div className="flex items-start justify-between mb-2">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <div className="flex flex-col gap-1">
                ...
                >−</button>
                <button
                  onClick={() => { setAutoFitEnabled(true); setZoom(1); setPan({ x: 0, y: 0 }); }}
          ...
          <div className="relative rounded-lg overflow-hidden bg-white" style={{ height: 380 }}>
            ...
            >
              ...
              {visibleIngredients.map(ing => {
                ...
                const isHighlighted = highlightSearch && (ing.name.includes(highlightSearch) || ing.nameEn.toLowerCase().includes(highlightSearch.toLowerCase()));
                ...
                return (
                  ...
                  >
                    <circle cx={x} cy={y} r={nodeSize}
                      ...
                      stroke={isHighlighted ? '#ae3a04' : 'white'} strokeWidth={strokeW}
                    />
                    {showLabel && (
                      <text x={x + nodeSize + 3} y={y + 4}
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-white rounded-xl shadow-sm border border-[#e0c0b5]/30 p-4">
            ...
            <div className="mb-3">
              ...
              <input type="range" min={0} max={100} value={Math.round(t * 100)}
                onChange={e => setT(Number(e.target.value) / 100)} aria-label="Mix ratio"
              />
          ...
          <div className="bg-white rounded-xl shadow-sm border border-[#e0c0b5]/30 p-4">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-5">
              <RadarChart datasets={[
                { label: '当前', data: currentFlavor, color: '#ae3a04' },
                { label: ingA.name, data: ingA.flavor, color: '#8c7168', dashed: true },
              ]} size={180} />
          ...
          {corridorPanorama.map((stop, i) => {
            ...
            return (
              <motion.div key={stop.stop}
                ...
                style={{ background: isCurrent ? '#ffdbcf' : '#f5ece7', border: isCurrent ? '2px solid #ae3a04' : '2px solid transparent' }}
              ...
              >
                ...
                {isCurrent && <span className="text-[10px] font-bold text-[#ae3a04] mt-1.5">当前</span>}
              </motion.div>
            );
          })}
        </div>
...
function SearchableSelect({ value, onChange, options }: SearchableSelectProps) {
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="w-full px-2.5 py-1.5 rounded-lg bg-[#f5ece7] border-none text-xs text-[#2c2825] outline-none text-left flex items-center gap-2 hover:bg-[#efe6e2] transition"
      >
        <div
          className="w-4 h-4 rounded-full flex items-center justify-center text-white text-[8px] font-bold flex-shrink-0"
          style={{ background: selected ? CATEGORY_COLORS[selected.category] : '#8c7168' }}
        >
          {selected ? selected.name[0] : '?'}
        </div>
        <span className="font-medium truncate">{selected?.name || '选择...'}</span>
        <span className="ml-auto text-[#8c7168] text-[10px]">▾</span>
      </button>
      ...
      {open && (
        <>
          ...
          <div className="absolute z-20 w-full mt-1 bg-white rounded-lg shadow-lg border border-[#e0c0b5]/30 max-h-56 overflow-hidden">
            {/* Search input */}
            <div className="p-2 border-b border-[#e0c0b5]/20 sticky top-0 bg-white">
              <div className="relative">
                <Search size={10} className="absolute left-2 top-1/2 -translate-y-1/2 text-[#8c7168]" />
                <input
                  ref={inputRef}
                  type="text"
                  value={search}
                  ...
                  placeholder="搜索..."
                  className="w-full pl-6 pr-2 py-1 rounded bg-[#f5ece7] border-none text-[11px] text-[#2c2825] outline-none placeholder:text-[#8c7168]"
                />
              </div>
            </div>
            {/* Options list */}
              ...
              {filtered.map(i => {
                ...
                return (
                  <button
                    ...
                    style={isActive ? { background: '#f5ece7' } : undefined}
                  >
                    <div
                    ...
                    <span className={isActive ? 'font-semibold text-[#ae3a04]' : ''}>{i.name}</span>
                    <span className="ml-auto text-[#8c7168] text-[10px]">{i.nameEn}</span>
                  </button>
                );
              })}
              {filtered.length === 0 && (
                <div className="px-3 py-4 text-center text-[10px] text-[#8c7168]">没有匹配的食材</div>
              )}
            </div>
          </div>
        </>
