...
install_skill() {
    ...
    # 菜谱数据走 API；dishes/ 是开发原料，绝不应进入用户包。
    # 纵深防御：即便打包出错带进 dishes/，安装阶段也清掉它。
    strip_local_dishes "$dest_dir"
...
update_skill() {
    ...
    if curl -fsSL "$SKILL_URL" -o "$tmp_file"; then
        ...
        # 菜谱数据走 API；dishes/ 是开发原料，更新时同样清掉。
        strip_local_dishes "$dest_dir"
...
# 清除用户包内不应存在的 dishes/ 目录（菜谱原料）。
# 运行时菜谱数据全部走 API，dishes/ 是开发/构建原料；若打包或旧版本残留带进来，
# 安装阶段在此兜底删除，保证用户机器干净。
strip_local_dishes() {
    local dest_dir="$1"
    if [ -d "$dest_dir/dishes" ]; then
        local size
        size=$(du -sh "$dest_dir/dishes" 2>/dev/null | cut -f1)
        warn "检测到 dishes/ 目录（${size}）— 菜谱数据走 API，已清理本地原料"
        rm -rf "$dest_dir/dishes"
    fi
}
